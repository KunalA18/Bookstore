...
<body>
    ...
    <?php
    include "config.php";
    // Get the book PID from the url query
    $PID = $_GET['ID'];
    $query = "SELECT * FROM products WHERE PID='$PID'";
    $result = mysqli_query($conn, $query) or die(mysqli_error($conn));

    if (mysqli_num_rows($result) > 0) {
        // For each book with give PID fetch its image and set the URL to cart
    ...
    <div class="row">
      ...
      <div class="col-sm-10 col-md-4 col-md-offset-1">
        ...
        <hr>            
            ...
            echo '
            // Select how many of each book to buy. The max book that one can buy is
            // given by the `Available` attribute
                                <select id="quantity">';
